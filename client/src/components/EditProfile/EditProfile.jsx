...
const EditProfile = ({ setOpen }) => {
    ...
    uploadTask.on(
      ...
      () => {
        ...
        getDownloadURL(uploadTask.snapshot.ref).then(async (downloadURL) => {
          try {
            const updateProfile = await axios.put(`http://localhost:8801/api/users/${currentUser._id}`, {
  ...
  const handleDelete = async () => {
    const deleteProfile = await axios.delete(`http://localhost:8801/api/users/${currentUser._id}`);
    dispatch(logout());
    navigate("http://localhost:8801/api/signin");
  };
